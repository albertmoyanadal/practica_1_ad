...
- Nacional: és repeteix a totes les observacions, per tant no ofereix informació rellevant i l'hem eliminat del dataframe
...
Una vegada hem entés com tenim les dades estructurades, ens podem fer algunes preguntes sobre aquestes.

### Afecta de manera similar a homes o dones?
...
D'aquí podem observar que més o manco tenim la mateixa proporció d'homes i dones, per tant podem concloure en que no hi ha una diferència de gènere.


### Com van evolucionar les defuncions de la Covid-19 setmanalment?
...
cov_Total$SemanaDef <- c(1:54)

ggplot(cov_Total[-1,], aes(x = SemanaDef, y = DefTotal, group = DefTotal)) + geom_point(color = "red", size = 2) + labs(title = "Evolució de la pandèmia per setmanes")+ theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold", color = "black"))
```

Aquí podem veure que hi ha dues 3 ones on la malaltia va causar moltes morts, on les dues primeres varen ser més fortes que la darrera.
